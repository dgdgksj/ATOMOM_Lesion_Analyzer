...
class SkinLesionPipeline:
    ...
    def __load_ef_net(self,ef_configs):
        ...
        for i, config in enumerate(ef_configs):
            models[i] = EfficientNetClassifier(Config=config,device=config.device)
    ...
    def __load_yolo(self,yolo_configs):
        ...
        for i, config in enumerate(yolo_configs):
            models[i] = YoloSegmenter(config=config)
        return models
    def __load_mrcnn(self,mrcnn_configs):
        if not mrcnn_configs:
            return []
        ...
        for i, config in enumerate(mrcnn_configs):
            models[i] = MaskRCNNSegmenter(mrcnn_config=config())
        return models
    ...
    def save_exp(self):
        file_path = experiment_results_path("exp_results", "result.xlsx")
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        self.exp_wb.save(file_path)
...
if __name__ == '__main__':
    ...
    skin_lesion = SkinLesionPipeline(
