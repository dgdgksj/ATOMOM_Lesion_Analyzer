...
from app_config.paths import repo_path, runtime_cache_path
...
class YoloSegmenter:
    ...
    def __get_image_paths(self, image_path):
        self.Config.file_paths = collect_image_paths(image_path)
    def __draw_boexes(self,image, boxes):
        ...
        cropped_images = [None for _ in range(len(xyxy))]
        confidences = [None for _ in range(len(xyxy))]
        for i, box in enumerate(xyxy):
            lw = max(round(sum(image.shape) / 2 * 0.003), 2)
            p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
            confidences[i] = float(conf[i]) if len(conf) > i else 0.0
            ...
            cur_class = self.Config.model_names[int(cls[i])] if len(cls) > i else ''
        ...
        # raise Exception('\033[31m' + "object is not detected!!!", "image_path: " + image_path + '\033[0m')
            ...
            for i, result in enumerate(results):
                ...
                if visualize and result.masks is not None:
                    image = self.__draw_segmentation(image, segmentations=result.masks.data)
                # cv2.imshow("image")
                # cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    yolo_seg = YoloSegmenter(config=Config_yolo)
